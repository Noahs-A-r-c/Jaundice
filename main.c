...
#define DEVICE_NAME                     "Jaundice BLE Draft 2"                               /**< Name of device. Will be included in the advertising data. */
...
void saadc_callback(nrf_drv_saadc_evt_t const * p_event)
{
    if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
    {
        ...
        uint8_t value[SAADC_SAMPLES_IN_BUFFER*2];    // *2 , *4le degistirdim 
        uint16_t bytes_to_send;                      //   uint16_t bytes_to_send;
        ...
        uint8_t nus_string[300];    

        for (int i = 0; i < SAADC_SAMPLES_IN_BUFFER; i++)
        {
           bytes_to_send = sprintf(nus_string, "%d\r\n" ,  p_event->data.done.p_buffer[i]);
           }                                                  
...
// Change the broadcasted saadc channel to channel 4 over uart
void saadc_change_to_channel_4(void)
{
...
int concatenate_ints(int int1, int int2, int int3) {
  ...
  sprintf(str1, "%03d", int1);
  sprintf(str2, "%03d", int2);
  sprintf(str3, "%03d", int3);

  char resultstr[12];

  sprintf(resultstr, "%s%s%s", str1, str2, str3);
...
void led_timing(int full_interval_us, int duty1, int duty2, int period_us)
{
    ...
    led_1_on_time_us = (full_interval_us*duty1)/100; //Define time interval of LED1 based on duty cycle and full interval
    led_1_cycles = 2*led_1_on_time_us/period_us;
    for(i=1; i<=led_1_cycles; i++) // the amount of periods for this duty cycle of LED1 being on is the on 2*time / period.
    {
    ...
    leds_off_end_time_us = full_interval_us - led_1_on_time_us - led_2_on_time_us;
    nrf_gpio_pin_clear(LED1); //Clear all LED pins
    nrf_gpio_pin_clear(LED2);
    nrf_delay_us(leds_off_end_time_us);
}
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    ...
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        case 01:     
          nrf_gpio_pin_clear(LED1); // LED 1 ON
          nrf_gpio_pin_set(LED2); // LED 2 OFF
        break;
        //if 0x02 is sent over BLE turn LED 2 on
        case 02:
          nrf_gpio_pin_set(LED1); // LED 1 OFF
          nrf_gpio_pin_clear(LED2); // LED 2 ON
        break;
        //if 0x21 "!" is sent over BLE turn LEDs 1 and 2 on
        case 33:
         nrf_gpio_pin_clear(LED1); // LED 1 ON
         nrf_gpio_pin_clear(LED2); // LED 2 ON
        break;
...
static void leds_init(void)
{
    ...
    nrf_gpio_cfg_output(LED1); //Configures pin as output pin
